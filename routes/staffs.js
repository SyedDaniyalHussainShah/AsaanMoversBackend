...
const { Staff, validate, validatePassword } = require('../models/staff');
...
router.post('/', async (req, res) => {
    ...
    if(req.body.type != "DRIVER" && req.body.type != "LABOURER"){
...
router.post('/update',adminAuth, async (req, res) => {
    ...
    const staff = await User.findByIdAndUpdate(req.body._id,
      { 
        name: req.body.name,
        email: req.body.email,
        contact: req.body.contact,
   ...
   // if (!customer) return res.status(404).send('The customer with the given ID was not found.');
    ...
    res.send(_.pick(staff, ['_id', 'name', 'email', 'contact']));
  });

  
  router.post('/delete',adminAuth ,async (req, res) => {
    const customer = await User.findByIdAndRemove(req.body._id);
